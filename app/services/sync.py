...
def run_mitre_sync():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"🗑️ Cleared {cleared} stale threat actors for fresh sync.")
            ...
            if octi_url and octi_token:
                ...
                try:
                    ...
                    if not df_octi.empty:
                        ...
                        logger.warning("⚠️ No actors returned from OpenCTI.")
                except Exception as e:
                    logger.error(f"❌ OpenCTI sync failed: {e}")
                    import traceback
                    logger.error(traceback.format_exc())
            else:
                logger.info("ℹ️ OpenCTI not configured, skipping.")
