...
### Added
- **Startup auth-source banner (`app/main.py` lifespan):** Logs, in plain English, every credential source TIDE will use on the next sync \u2014 number of `siem_inventory` rows, how many are active+have-token+have-url, whether the legacy `ELASTIC_URL` / `ELASTIC_API_KEY` env vars are set, the shared `detection_rules` row count, and how many distinct SIEMs appear in `client_siem_map`. For each SIEM also logs label, active flag, URL and `token_present=True/False`. When NO credentials exist anywhere it raises a WARN that points at the diagnostic script. This single banner answers "is sync going to use the env vars or the DB record?" \u2014 a question that historically required reading three files and a `docker exec env` to figure out.

- **`test/diag_sync.py` \u2014 end-to-end sync diagnostic.** Run with `docker exec tide-app python /app/test/diag_sync.py`. Walks the full `env -> shared DB -> SIEM record -> Kibana` chain and reports, per step, what's there and what's missing. Handles 4.0.x (env-var driven), 4.0.13+ (`siem_inventory`) and 4.1.x (per-tenant DB) without needing to know which version is running. Hits each Kibana endpoint with the actual stored token (no TIDE wrappers, raw `requests`) so HTTP 200/401/403/404 results reflect only credentials and network. Token values are redacted to `len=N first=AAAA last=BBBB`. Ends with a verdict block that names the failing link in the chain and the exact next action.

- **`test/repair_redistribute.py` \u2014 manual rule push.** Run with `docker exec tide-app python /app/test/repair_redistribute.py`. Runs the same code path as the lifespan hook and the SIEM link/unlink endpoints (`_distribute_rules_to_tenants`); idempotent and safe to invoke any number of times. Use when shared `detection_rules` is populated but a tenant's UI shows zero rules. No-ops cleanly on 4.0.x where rules live in the shared DB.

### Fixed
- **Migration 37 banner pointed at non-existent UI (`app/templates/base.html`).** Copy said "Trigger a sync from Settings (or call POST /api/admin/sync)". Neither exists in this version: there is no Settings page, and the actual endpoint is `POST /api/sync/elastic`. Operators reading the banner had nowhere to go. Updated to point at the **Sync Elastic** button on the Dashboard and the **Sync from Elastic** button on the Rules page (both routes are linked inline). Also dropped the outdated "for every tenant" wording \u2014 since the check is now tenant-aware (see fix below) the banner only fires for the active tenant.
