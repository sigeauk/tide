...
### Fixed
- **`list_siem_inventory()` returned dropped columns (`app/services/database.py`).** Before Migration 38 ran, this helper SELECTed `production_space` and `staging_space` and the templates rendered them. Cleaned up to match the new schema; the `_siem_logging_block_html` partial in `app/api/management.py` similarly stopped reading those keys so the SIEMs page no longer 500s post-migration.
